...
output=$(python3 positions2D.py $L $phi $x0 $mono_diam $wall_diam $separation)
...
then
    ...
    sed -i "s/[0-9]\+ atoms/${output} atoms/" dataFormat2D.txt

    cat "dataFormat2D.txt" "../Data/brownian_L_${L}_phi_${phi}_2D.data" > tmp && mv tmp "../Data/brownian_L_${L}_phi_${phi}_2D.data"
fi